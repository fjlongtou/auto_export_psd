...
async function saveImage(imageData, filePath) {
    return new Promise((resolve, reject) => {
        ...
        writeStream.on('finish', () => {
            console.log(`图片保存完成: ${filePath}`);
            resolve();
        });
        
        writeStream.on('error', (err) => {
            console.error(`图片保存失败: ${filePath}`, err);
            reject(err);
        });

        imageData.saveAsPng(filePath)
...
/**
 * Convert a PSD file to a fairygui package and import to project.
 * @param {string} psdFile path of the psd file.
 * @param {string} projectPath FairyGUI project path.
 * @param {string} packageName package name in project.
 * @param {integer} option psd2fgui.constants.
 * @param {string} buildId optional. You can use same build id to keep resource ids unchanged.
 * @return {Promise} resolves with buildId
 */
exports.convertToProject = function(psdFile, projectPath, packageName, option, buildId) {
    return new Promise(async function(resolve, reject) {
        if (!fs.existsSync(projectPath)) {
            reject(new Error('Project path not exists: ' + projectPath));
            return;
        }
        ...
        try {
            ...
            targetPackage = new UIPackage(projectPath, buildId);
            targetPackage.exportOption = option;

            createComponent(psd.tree(), path.parse(psdFile).name);
            ...
            // 处理所有资源
            for (const item of targetPackage.resources) {
                var resNode = resourcesNode.ele(item.type);
                resNode.att('id', item.id).att('name', item.name).att('path', '/');
                ...
                if (item.type == 'image') {
                    ...
                    // 等待图片保存完成
                    console.log(`开始保存图片: ${item.name}`);
                    await saveImage(item.data, path.join(targetPackage.basePath, item.name));
                } else {
                    await fs.writeFile(path.join(targetPackage.basePath, item.name), item.data);
                    console.log(`保存文件: ${item.name}`);
                }
            }

            // 保存配置文件
            await fs.writeFile(
                path.join(targetPackage.basePath, 'package.xml'),
                pkgDesc.end({ pretty: true })
            );
            console.log('保存配置文件: package.xml');

            // 等待文件系统缓冲区刷新
            await new Promise(resolve => setTimeout(resolve, 500));

            console.log(`转换完成: ${psdFile} -> ${projectPath}`);
            resolve(buildId);
...
exports.run = function(argv) {
    if (argv.length == 0) {
        ...
        console.info('   or: psd2fgui --project projectPath --package packageName psdFile [--ignore-font] [#buildId]');
        process.exit(0);
    }
    ...
    for (var i = 0; i < argv.length; i++) {
        ...
        if (arg.indexOf('--') == 0) {
            switch (arg.substr(2)) {
                case 'nopack':
                    option |= exports.constants.NO_PACK;
                    break;
                case 'ignore-font':
                    option |= exports.constants.IGNORE_FONT;
                    break;
                case 'project':
                    if (i + 1 < argv.length) {
                        projectPath = argv[++i];
    ...
    if (!psdFile) {
        console.error('Missing PSD file');
        process.exit(1);
    }

    // 如果指定了项目路径和包名，使用项目导入模式
    if (projectPath && packageName) {
        exports.convertToProject(psdFile, projectPath, packageName, option, buildId)
        ...
        // 原有的转换模式
        exports.convert(psdFile, outputFile, option, buildId)
            .then(function(buildId) {
                console.log('buildId: ' + buildId);
            })
            .catch(function(err) {
